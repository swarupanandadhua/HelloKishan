...
TAGS:
    ...
    {
    Generate:
        keytool - genkey - v - keystore ~ /.android / debug.keystore - storepass android - alias androiddebugkey - keypass android - keyalg RSA - keysize 2048 - validity 10000 Print : keytool - list - v - alias androiddebugkey - keystore ~ /.android / debug.keystore
...
{
Vision:
    To provide access to buy and sell agricultural products in 1 click
        Mission : 1. To balance out price difference in the agricultural market 2. To maximize the gain of Farmers and minimize the hassle 3. To minimize the expense of a household on vegetables
}

4 pillars of a company{
    Engineering(Frontend, Backend, Security, Data Science)
        Sales-- --------------handles customer,
    complaints etc
...
Django : https : //www.django-rest-framework.org/
                 ...
                 Django 1 {// https://dev.to/amartyadev/flutter-app-authentication-with-django-backend-1-21cp
                           Django REST Framework(DRF) : PostgreSQL in Django + SQLite in Frontend
                                                                                   ...
                                                                                   BLoC works on sinks and
                           streams : The widgets monitor state changes & send them to BloC using sinks & other widgets monitor those by subscribing to streams.
